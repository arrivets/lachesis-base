...
func BenchmarkFlushableNoFlush(b *testing.B) {
	disk := dbProducer("BenchmarkFlushable")
		...
		for goroutines := 1; goroutines <= recs; goroutines *= 2 {
			for reading := 0; reading <= 1; reading++ {
				name := fmt.Sprintf(
					...
					goroutines, flushPeriod, reading)
				b.Run(name, func(b *testing.B) {
					benchmarkFlushableNoFlush(flushable, goroutines, b.N, flushPeriod, reading != 0)
				})
			}
		}
	}
...
func benchmarkFlushableNoFlush(db *Flushable, goroutines, recs, flushPeriod int, readingExtensive bool) {
	leftRecs := recs
	var wg sync.WaitGroup
	wg.Add(goroutines)
	for i := 0; i < goroutines; i++ {
