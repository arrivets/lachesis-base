// This is taken from go-ethereum:
// https://raw.githubusercontent.com/ethereum/go-ethereum/master/ethdb/memorydb/memorydb.go

// Package memstore implements the key-value database layer based on memory maps.
package memstore
...
func (db *Database) Delete(key []byte) error {
	...
	if val != nil {
		...
		db.sizeEstimation -= (len(key) + len(val))
	}
...
// GetSnapshot creates a database snapshot based on the current state.
...
func (db *Database) SizeEstimation() int {
	db.lock.RLock()
	defer db.lock.RUnlock()

	return db.sizeEstimation
}
...
func (snap *snapshot) NewIterator(prefix []byte, start []byte) kvdb.Iterator {
	...
	var (
		...
		values = make([][]byte, 0, len(snap.db))
	)
	// Collect the keys from the memory database corresponding to the given prefix
	// and start
	for key := range snap.db {
		if !strings.HasPrefix(key, pr) {
			continue
		}
		if key >= st {
	...
	sort.Strings(keys)
	for _, key := range keys {
		values = append(values, snap.db[key])
	}
	return &iterator{
		index:  -1,
		keys:   keys,
